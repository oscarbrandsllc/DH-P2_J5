    ...
    hideLegend();
        ...
        function renderGameLogs(gameLogs, player, playerRanks) {
            ...
            if (gameLogsWithData.length > 0) {
                ...
                totalTh.innerHTML = `<span class="season-label">SEASON</span><br><span class="gp-label">(GP: ${gamesPlayed})</span>`;
        ...
        function renderPlayerComparison(players) {
            ...
            players.forEach(player => {
                ...
                nameButton.onclick = () => {
                    ...
                    const basePos = player.basePos || player.pos || fullPlayer?.position || '';
                    const rosterStyleData = getPlayerData(player.id, basePos);
                    const resolvedTeam = (player.team || rosterStyleData.team || fullPlayer?.team || 'FA').toUpperCase();

                    const mergedPlayerData = {
                        ...rosterStyleData,
                        id: player.id,
                        name: playerName,
                        pos: (player.pos || rosterStyleData.pos || basePos || '').toUpperCase(),
                        team: resolvedTeam,
                        ktc: player.ktc ?? rosterStyleData.ktc ?? null,
                        overallRank: player.overallRank ?? rosterStyleData.overallRank ?? null,
                        posRank: player.posRank ?? rosterStyleData.posRank ?? null
                    };
