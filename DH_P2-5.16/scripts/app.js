...
function showLegend(){ try{ document.getElementById('legend-section')?.classList.remove('hidden'); }catch(e){} }
    ...
    hideLegend();
        ...
        const STAT_KEY_RANK_OVERRIDES = { fpts: 'pos_rank_ppr', fpts_ppr: 'pos_rank_ppr' };
            ...
            for (const statKey of orderedStatKeys) {
                if (statLabels[statKey]) {
                    ...
                    for (let i = 0; i < players.length; i++) {
                        ...
                        const rankAnnotation = createRankAnnotation(rankValue);
                        const numericRank = typeof rankValue === 'number' && Number.isFinite(rankValue) ? rankValue : null;

                        displayValues.push(displayValue);
                        rankAnnotations.push(rankAnnotation);

                        if (numericRank !== null) {
                            if (numericRank < bestRank) {
                                bestRank = numericRank;
                                bestRankIndices = [i];
                            } else if (numericRank === bestRank) {
                    ...
                    displayValues.forEach((val, i) => {
                        ...
                        const hasRankComparison = bestRankIndices.length > 0 && Number.isFinite(bestRank);
                        const highlightIndices = hasRankComparison ? bestRankIndices : maxIndices;
                        const isTie = highlightIndices.length > 1;
                        if (val !== 'N/A' && highlightIndices.includes(i)) {
                            if (isTie) {
