...
function showLegend(){ try{ document.getElementById('legend-section')?.classList.remove('hidden'); }catch(e){} }
    ...
    hideLegend();
        ...
        function formatRankSuperscript(rank) {
            if (rank === null || rank === undefined || Number.isNaN(rank)) {
                return '⁽ᴺᴬ⁾';
            }

            const rankStr = String(rank);
            if (!rankStr.trim()) return '⁽ᴺᴬ⁾';
            const upper = rankStr.trim().toUpperCase();
            if (upper === 'NA' || upper === 'N/A') return '⁽ᴺᴬ⁾';

            const superscript = rankStr.split('').map(char => {
                if (SUPERSCRIPT_DIGITS[char] !== undefined) return SUPERSCRIPT_DIGITS[char];
                if (char === '-') return '⁻';
                if (char === '+') return '⁺';
                return char;
            }).join('');
        ...
        function renderGameLogs(gameLogs, player, playerRanks) {
            ...
            if (gameLogsWithData.length > 0) {
                ...
                for (const key of orderedStatKeys) {
                    ...
                    const rankSuperscript = formatRankSuperscript(rankValue);
                    td.textContent = `${displayValue}${rankSuperscript}`;
            ...
            for (const statKey of orderedStatKeys) {
                if (statLabels[statKey]) {
                    ...
                    for (let i = 0; i < players.length; i++) {
                        ...
                        rankSuperscripts.push(rankSuperscript);
                    ...
                    displayValues.forEach((val, i) => {
                        const td = document.createElement('td');
                        td.textContent = val;
                        td.textContent += rankSuperscripts[i] || '';
                        if (val !== 'N/A') {
